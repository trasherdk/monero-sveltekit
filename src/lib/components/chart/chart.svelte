<script>
	...
	const candlestickSeries = fc.autoBandwidth(fc.seriesSvgOhlc()).xScale(xScale).yScale(priceScale);
	...
	const volumeAxis = d3.axisRight().scale(volumeScale).ticks(3).tickFormat(d3.format(',.3s'));
	...
	d3.select('#x-axis').on('draw', (d, i, nodes) => {
		d3.select(nodes[i]).select('svg').call(xAxis);
	});
	...
	d3.select('#y-axis')
		...
		.on('draw', (d, i, nodes) => {
			const sel = d3.select(nodes[i]).select('svg').selectAll('g').data([0]).enter();
	...
	d3.csv(
		...
		d3.select('#plot-area').on('draw', (d, i, nodes) => {
			const sel = d3.select(nodes[i]).select('svg').selectAll('g').data([data]).enter();
			sel.append('g').classed('volume', true).call(volumeSeries);
	...
	<d3fc-group id="chart" style="display: flex; height: 100%; width: 100%; overflow: hidden">
		<div style="flex: 1; display: flex; flex-direction: column; padding-right: 1em">
			<div style="flex: 1; display: flex; flex-direction: row">
				...
				<d3fc-svg id="y-axis" style="width: 1em" />
				<d3fc-svg id="plot-area" style="flex: 1; overflow:hidden" />
			</div>
			<d3fc-svg id="x-axis" style="height: 2em; margin-left: 2em" />
		</div>
	</d3fc-group>
</div>
